...
class JSONFilter:
    ...
    @staticmethod
    def filterNonNormalizedField(
        jsonEvent: Union[List[Dict], Dict],
        field: str
    ...
    def filterNonNormalizedEvent(self, jsonEvent: Union[List[Dict], Dict]) -> Dict:
    ...
    def filterNormalizedEvent(self, jsonEvent: Union[List[Dict], Dict]) -> DataFrame:
    ...
    def filter(self, jsonEvents: Union[List[Dict], Dict]) -> Union[List[DataFrame], List[Dict]]:
    ...
    def filter_and_concat(self, jsonEvents: Union[List[Dict], Dict]) -> Dict:
...
class JSONTokenizer:
    ...
    def clear_json_event(self, text: Union[str, bytes, list, dict]) -> str:
        ...
        """
        assert isinstance(text, (str, bytes, list, dict)), "Input must be a string, bytes, list, or dict!"
        if isinstance(text, bytes):
            text = text.decode('utf-8')
        text = ' '.join(text) if isinstance(text, list) and isinstance(text[0], str) else str(text)
        text = text.lower()
...
class JSONTokenizerBPE(JSONTokenizer):
    ...
    def split_string_to_chunks(self, s: str, chunkSize: int = 4192):
